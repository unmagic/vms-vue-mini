...
export type PageContext = WechatMiniprogram.Page.InstanceProperties &
  Omit<
    ...
    | 'getRelationNodes'
  >
export type PageSetup = (this: void, query: Query, context: PageContext) => Bindings
...
export function definePage(optionsOrSetup: any, config?: Config): void {
  ...
  options[PageLifecycle.ON_READY] = createLifecycle(options, PageLifecycle.ON_READY)
  options[PageLifecycle.ON_HIDE] = createLifecycle(options, PageLifecycle.ON_HIDE)
  options[PageLifecycle.ON_ROUTE_DONE] = createLifecycle(options, PageLifecycle.ON_ROUTE_DONE)
  options[PageLifecycle.ON_PULL_DOWN_REFRESH] = createLifecycle(
    options,
    PageLifecycle.ON_PULL_DOWN_REFRESH,
  )
  options[PageLifecycle.ON_REACH_BOTTOM] = createLifecycle(options, PageLifecycle.ON_REACH_BOTTOM)
  options[PageLifecycle.ON_RESIZE] = createLifecycle(options, PageLifecycle.ON_RESIZE)
  options[PageLifecycle.ON_TAB_ITEM_TAP] = createLifecycle(options, PageLifecycle.ON_TAB_ITEM_TAP)

  Page(options)
}

function createLifecycle(options: Options, lifecycle: PageLifecycle): (...args: any[]) => void {
